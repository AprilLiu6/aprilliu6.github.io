...
安装好这两个软件之后，我们首选把我们在我的仓库里面创建的项目整个下载下来

![](../img/2019052615.png)

下载下来是一个压缩文件，进行解压，然后打开VScode，打开整个文件夹，也可以直接整个文件夹拉进去，就可以了。

![](../img/2019052617.png)

可以看到，在VScode里面左边的所有文件目录，和我们在我的仓库里面看到的是一样的

接下来我们都只需要在VScode里面进行操作就可以了。

首先我们把原作者发布的文件全部删除，如果你想保留一些作为参考备用也可以部分保留
...
在VScode我们的项目文件里面，打开 `_posts` 的文件夹
...
由于我们创建博客文章使用了VScode，所以这里的所有操作都是在VScode里面对应的位置进行修改的。

到这里基本就掌握了自己写博客文章的基本方法了，终于有一种要完工的感觉。

我也是边学边写，写到这里我才发现自己的第一篇文章终于快要写好了。

终于在`_posts` 里的`2019-05-26-新手如何快速搭建GitHub Pages.md`里面完成了我第一篇文章的90%

接下来就差用GitHub Desktop发布了。
...
终于要用到前面说的GitHub desktop了，下载安装好以后打开
...
接着，把我们在VScode里面进行过修改的所有文件夹，替换进这个克隆下来的文件夹里面，包括`_posts`文件夹、`img`文件夹等。

替换完成以后，在GitHub Desktop里面点击Commit。

后面发现最方便其实是一开始先用GitHub Desktop克隆项目文件夹，然后把这个文件夹拉进VScode直接编辑，这样我们所有的操作都会同步到GitHub Desktop里面。就不用多一步替换文件的操作了。
